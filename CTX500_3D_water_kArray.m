% CTX-500 4-element transducer for k-wave simulations in 3D
tic;
...
% driving parameters
source_amp      = repmat(85154.3, [1,4]); % source pressure [Pa] for 54.49 W/cm2 Isppa
source_phase    = deg2rad([0	319	278	237]); % phase [rad]
% source_phase    = deg2rad([0.00	69.00	138.00	207.00]);	% phase [rad]

% aperture diameters of the elements given an inner, outer pairs [m]
in2m = 0.0254;	% conversion from inches to m (element dimensions given in inches)
diameters       = [0 1.28; 1.3 1.802; 1.82 2.19; 2.208 2.52].' *in2m;
...
% sensor.record = {'p', 'p_max_all', 'p_rms', 'p_final'};
sensor.record = {'p'};
...
figure;
imagesc(1e3 * x_vec, 1e3 * y_vec, p(:, :, Nz/2 +1).');
colormap('turbo');
xlabel('Axial Position [mm]');
...
% subplot(2, 2, 1);
% imagesc(kgrid.x_vec * 1e3, kgrid.y_vec * 1e3, (p(:,:,Nz/2+1) * 1e-6).');
% h = colorbar;
% xlabel(h, '[MPa]');
...
% subplot(2, 2, 2);
% imagesc(kgrid.x_vec * 1e3, kgrid.y_vec * 1e3, (Q(:,:,Nz/2+1) * 1e-7).');
% h = colorbar;
% xlabel(h, '[kW/cm^2]');
% ylabel('y-position [mm]');
% xlabel('x-position [mm]');
...
% subplot(2, 2, 3);
% imagesc(kgrid.x_vec * 1e3, kgrid.y_vec * 1e3, T1(:,:,Nz/2+1).');%, [37, 37.1]);
% h = colorbar;
% xlabel(h, '[degC]');
% ylabel('y-position [mm]');
% xlabel('x-position [mm]');
...
% disp(['MI = ' num2str(MI)])
% disp(['Coordinates of max pressure: ' num2str(mx) ', ' num2str(my) ', ' num2str(mz)])
% disp(['Distance from transducer rear surface: ' num2str(norm([bx,65,65]-[mx,my,mz])*dx*1e3) 'mm'])
% disp(['Isppa = ' num2str(Isppa) ' W/cm2'])
% disp(['Ispta = ' num2str(Ispta*1e3) ' mW/cm2'])
